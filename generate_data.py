...
from model_scripts.score_pitches import predict_with_pitch_value_models
...
def update_statcast_year(year, overwrite=False, cache=True):
    ...
    if overwrite:
        ...
        try:
            curr_year_data = pd.read_parquet(f'pitches_by_season/pitches_{year}.parquet')
            start_date = str((curr_year_data.game_date.max() + timedelta(1)).date())
...
if __name__=='__main__':
    ...
    if True:
        ...
        df = load_statcast_years(year_start=2024, year_end=2024)

        k_pitch_type_map = pd.read_parquet(f'intermediate_data_files/{model_run_name}/k_pitch_type_map.parquet')
        df, _ = engineer_features(df,f'pitch_type_models/{model_run_name}', 
                                  fit_pipeline=False, k_pitch_type_map = k_pitch_type_map)
        ...
        data_with_pitch_values = predict_with_pitch_value_models(df, path=f'pitch_value_models/{model_run_name}')
